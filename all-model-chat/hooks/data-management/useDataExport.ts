...
export const useDataExport = ({
    ...
    const handleExportSettings = useCallback(() => {
        ...
        try {
            ...
            triggerDownload(URL.createObjectURL(blob), `my-ai-front-settings-${date}.json`);
        } catch (error) {
    ...
    const handleExportHistory = useCallback(async () => {
        ...
        try {
            ...
            const blob = new Blob([jsonString], { type: 'application/json' });
            const date = new Date().toISOString().slice(0, 10);
            triggerDownload(URL.createObjectURL(blob), `my-ai-front-history-${date}.json`);
        } catch (error) {
    ...
    const handleExportAllScenarios = useCallback(() => {
        ...
        try {
            ...
            const blob = new Blob([jsonString], { type: 'application/json' });
            const date = new Date().toISOString().slice(0, 10);
            triggerDownload(URL.createObjectURL(blob), `my-ai-front-scenarios-${date}.json`);
        } catch (error) {
